...
while ($row = mysql_fetch_assoc($ogrenciler)) {
	...
	echo '<td><button class="btn" onclick=" window.open(\'./ogrenciduzenle.php?id='.$row['id'].'\',\'_blank\')"><i class="fa fa-refresh"></i></button> <button class="btn" id="ogrenciid"  onclick="sil()" value="'.$row['id'].'"><i class="fa fa-trash"></i></button></td>';
	$i++;
}
}